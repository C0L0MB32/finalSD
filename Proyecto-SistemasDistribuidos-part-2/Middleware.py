...
def disposAgregar(data, num):
    try:
        ...
        partes = data.split(',')
        # Buscar la posición del primer elemento después de "AGREGAR"
        index_agregar = partes.index('AGREGAR')
        # Los datos comienzan después de "AGREGAR", así que los recogemos desde la siguiente posición
        datos = [parte.strip() for parte in partes[index_agregar + 1:]]
